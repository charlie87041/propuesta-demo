...
**Descripción**: Registro de nuevos usuarios con asignación automática de abilities ABAC.
...
1. **Red Phase**:
   ...
   - **ABAC Integration tests**:
     - Test registro asigna abilities de customer al dominio "main-store"
     - Test customer registrado tiene `browse-catalog` ability
     - Test customer registrado tiene `manage-cart` ability
     - Test customer registrado tiene `checkout` ability
     - Test customer registrado tiene `manage-profile` ability
     - Test customer registrado tiene `view-orders` ability
     - Test customer NO tiene abilities de admin

2. **Green Phase**:
   - Crear `RegistrationService`
   - Crear `EmailVerificationService`
   - Integrar con `DomainAuthorizationService` para asignar abilities
   - Crear `CustomerAbilityAssignmentService`

3. **Refactor Phase**:
   - Extraer ability assignment a evento de dominio
   - Configuración de abilities por defecto en properties

**Acceptance Criteria**:
- [ ] Registro exitoso
- [ ] Validación de email único
- [ ] Email verification funciona
- [ ] Customer recibe abilities por defecto automáticamente
- [ ] Abilities asignadas al dominio correcto
...
### Task 8.4: Implement Admin User Entity and Repository

**Descripción**: Entidad AdminUser separada de Customer para usuarios administrativos.

**Enfoque TDD**:
1. **Red Phase**:
   - Test crear admin user con email único
   - Test encontrar por email
   - Test password se hashea con BCrypt
   - Test admin user diferente de customer entity
   - Test admin user puede tener múltiples domain assignments

2. **Green Phase**:
   - Crear entidad `AdminUser`
   - Crear `AdminUserRepository`
   - Relacionar con `UserDomainAbility` y `UserDomainPermissionOverride`

**Acceptance Criteria**:
- [ ] AdminUser separado de Customer
- [ ] Emails únicos
- [ ] Passwords hasheados con BCrypt
- [ ] Relación con authorization entities

**Related Requirements**: R11

---

### Task 8.5: Implement Admin User Management Service

**Descripción**: Servicio para gestión de usuarios administrativos con ABAC.

**Enfoque TDD**:
1. **Red Phase**:
   - Test crear admin user
   - Test asignar ability a admin user en domain
   - Test revocar ability de admin user
   - Test asignar permission override (allow/deny)
   - Test listar admin users por domain
   - Test listar abilities de admin user
   - Test listar permissions efectivos de admin user
   - Test desactivar admin user revoca todas las abilities
   - Test no puede auto-revocar última ability de super-admin

2. **Green Phase**:
   - Crear `AdminUserService`
   - Crear `AdminAbilityAssignmentService`
   - Integrar con `DomainAuthorizationService`

3. **Refactor Phase**:
   - Agregar eventos de dominio para cambios de permisos
   - Invalidar cache de permisos al modificar

**Acceptance Criteria**:
- [ ] CRUD de admin users
- [ ] Asignación/revocación de abilities funciona
- [ ] Permission overrides funcionan
...
### Task 8.6: Implement Admin User REST Controllers

**Descripción**: Endpoints para gestión de usuarios administrativos.

**Enfoque TDD**:
1. **Red Phase**:
   - Test GET /api/domains/{domainCode}/admin/users lista admin users
   - Test POST /api/domains/{domainCode}/admin/users crea admin user
   - Test GET /api/domains/{domainCode}/admin/users/{id} obtiene detalle
   - Test PUT /api/domains/{domainCode}/admin/users/{id} actualiza
   - Test DELETE /api/domains/{domainCode}/admin/users/{id} desactiva
   - Test POST /api/domains/{domainCode}/admin/users/{id}/abilities asigna ability
   - Test DELETE /api/domains/{domainCode}/admin/users/{id}/abilities/{abilityId} revoca
   - Test POST /api/domains/{domainCode}/admin/users/{id}/permissions/override crea override
   - **Authorization tests (Domain-Ability-Permission)**:
     - Test sin auth retorna 401
     - Test customer retorna 403
     - Test admin sin `manage-customers` ability retorna 403
     - Test admin con `manage-customers` puede listar users
     - Test solo `super-admin` puede asignar abilities de admin
     - Test no puede asignar abilities que no posee
     - Test domain isolation (403 para domain incorrecto)

2. **Green Phase**:
   - Crear `AdminUserController` con path `/api/domains/{domainCode}/admin/users`
   - Agregar `@RequiresPermission` y `@RequiresAbility` annotations
   - Implementar validation de ability hierarchy

**Acceptance Criteria**:
- [ ] CRUD endpoints funcionan
- [ ] Ability assignment/revocation endpoints funcionan
- [ ] Permission override endpoints funcionan
- [ ] Strict authorization enforcement
- [ ] Domain isolation funciona
- [ ] 401/403 responses correctos
- [ ] No puede escalar privilegios más allá de los propios

**Related Requirements**: R11

---

### Task 8.7: Implement Admin Customer Management

**Descripción**: Gestión de customers desde panel admin.

**Enfoque TDD**:
1. **Red Phase**:
   - Test GET /api/domains/{domainCode}/admin/customers lista customers
   - Test GET /api/domains/{domainCode}/admin/customers/{id} obtiene detalle
   - Test PUT /api/domains/{domainCode}/admin/customers/{id} actualiza
   - Test POST /api/domains/{domainCode}/admin/customers/{id}/disable desactiva
   - Test POST /api/domains/{domainCode}/admin/customers/{id}/enable reactiva
   - Test GET /api/domains/{domainCode}/admin/customers/{id}/orders lista órdenes
   - **Authorization tests**:
     - Test sin auth retorna 401
     - Test customer retorna 403
     - Test admin sin `manage-customers` ability retorna 403
     - Test admin con `manage-customers` puede listar customers
     - Test `customers:list` permission requerido para GET list
     - Test `customers:disable` permission requerido para disable
     - Test domain isolation funciona

2. **Green Phase**:
   - Crear `AdminCustomerService`
   - Crear `AdminCustomerController` con path `/api/domains/{domainCode}/admin/customers`
   - Agregar `@RequiresPermission` annotations

**Acceptance Criteria**:
- [ ] Lista de customers con paginación y filtros
- [ ] Detalle de customer con órdenes
...
| 3 | 3.1-3.5 | Catalog |
| 4 | 4.1-4.5 | Customer + **ABAC Registration** |
| 5 | 5.1-5.4 | Cart |
| 6 | 6.1-6.5 | Order |
| 7 | 7.1-7.4 | Payment |
| 8 | 8.1-8.7 | Admin + **User Management** |
| 9 | 9.1-9.5 | Frontend |
| 10 | 10.1-10.3 | Security & Perf |

**Total Tasks**: 46 (4 authorization + 4 user management tasks)
**Estimated Duration**: 8-10 weeks for MVP

### Authorization Tasks (Milestone 2)
...
### Customer ABAC Integration (Milestone 4)

| Task | Description | Effort |
|------|-------------|--------|
| 4.3 | Registration Service with ABAC ability assignment | 1.5 days |

### Admin User Management Tasks (Milestone 8)

| Task | Description | Effort |
|------|-------------|--------|
| 8.4 | Admin User Entity and Repository | 0.5 days |
| 8.5 | Admin User Management Service | 1.5 days |
| 8.6 | Admin User REST Controllers | 1 day |
| 8.7 | Admin Customer Management | 1 day |

### Authorization Tests Added to Existing Tasks

| Task | Authorization Tests Added |
|------|--------------------------|
| 4.3 | ABAC ability assignment on registration |
