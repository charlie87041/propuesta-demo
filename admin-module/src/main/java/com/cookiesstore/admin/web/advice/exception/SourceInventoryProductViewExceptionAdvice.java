...
public class SourceInventoryProductViewExceptionAdvice {

    private final MessageSource messageSource;

    public SourceInventoryProductViewExceptionAdvice(MessageSource messageSource) {
    ...
    @ExceptionHandler({SourceDomainException.class, ProductDomainException.class})
    public ModelAndView handleDomain(
        ...
        FlashMap flashMap = RequestContextUtils.getOutputFlashMap(request);
        String key = ex instanceof SourceDomainException sourceEx ? sourceEx.getMessageKey() : ((ProductDomainException) ex).getMessageKey();
        flashMap.put("errorMessage", message(key));
        RequestContextUtils.getFlashMapManager(request).saveOutputFlashMap(flashMap, request, response);
    ...
    private String resolveRedirectPath(HttpServletRequest request) {
        ...
        return "/admin/product-sources/" + sourceId + "/manage/inventory";
    }
